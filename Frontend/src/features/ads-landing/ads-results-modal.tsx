...
export function AdsResultsModal({
  open,
  loading,
  error,
  offers,
        ...
        <header className="border-b border-border px-6 py-5 sm:px-8">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              className="flex size-10 shrink-0 items-center justify-center rounded-[12px] border border-border text-secondary-text transition-colors hover:border-aviation-blue/50 hover:text-primary-text"
            >
              <CloseIcon />
            </button>
          </div>
          ...
          {loading ? (
            <div className="space-y-0 divide-y divide-border">
              <div className="py-6 xl:hidden">
                <div className="h-40 animate-pulse rounded-[12px] bg-soft-section" />
              </div>
              {Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="hidden py-6 xl:block">
                  <div className="h-32 animate-pulse rounded-[12px] bg-soft-section" />
                </div>
              ))}
            </div>
          ) : null}
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-secondary-text">
                {searchContext
                  ...
                  : "A specialist tickets this fare by phone."}
              </p>
            </div>
            <CallPhoneButton
              size="sm"
