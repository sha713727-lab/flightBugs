...
export function AdsSearchPanel() {
  ...
  const [from, setFrom] = useState<FlightSearchPlace>(adsLandingDefaultFrom);
  const [to, setTo] = useState<FlightSearchPlace>(adsLandingDefaultTo);
  ...
  if (destinationCode !== appliedDestination) {
    ...
    if (destinationCode) {
      ...
      if (place) {
        setTo(place);
        setFrom((current) =>
          current.iata === place.iata ? adsLandingDefaultTo : current,
        );
      }
    }
  }
  ...
  return (
    <>
      <div className="ads-search-panel overflow-visible p-3 sm:p-5">
          ...
          <div className="flex flex-col">
            <div className="grid md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
              ...
              <div className="flex items-center justify-center py-1 md:px-1 md:py-0">
                <button
                  ...
                  className="flex size-10 shrink-0 rotate-90 items-center justify-center rounded-full border border-border bg-[var(--search-panel-bg)] text-primary-text transition-colors hover:border-aviation-blue/60 md:rotate-0"
                >
                  <SwapIcon />
                </button>
              </div>
            ...
            <div className="border-t border-border p-3">
              <button
                ...
                disabled={isPending}
              >
                {isPending ? "Searching..." : adsLandingCopy.searchCta}
              </button>
            </div>
          </div>
        )}
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="max-w-2xl space-y-2">
        ...
        <p className="text-sm leading-relaxed text-secondary-text">
          For three or more cities, call our desk — 24/7 — and a specialist will
